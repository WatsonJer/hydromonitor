<template>
  <v-container fluid bg-color="surface">
    <!--First row-->
    <v-row style="max-width: 1200px" class="p-1">
      <v-col>
...
const CreateCharts = async () => {
  ...
  tempHiChart.value = Highcharts.chart("container", {
    ...
    subtitle: {
      text: `The heat index, also known as the "apparent temperature," is a measure that combines air
temperature and relative humidity to assess how hot it feels to the human body. The relationship
between heat index and air temperature is influenced by humidity levels. As humidity increases, the heat
index also rises, making the perceived temperature higher than the actual air temperature.`,
  ...
  histogramCharts.value = Highcharts.chart("container1", {
    chart: { zoomType: "x", backgroundColor: "#1e1e1e" },
    title: {
  ...
  if (!!start.value && !!end.value) {
    ...
    data.forEach((row) => {
      graph1.push([
        ...
        parseFloat(row.heatindex.toFixed(2)),
      ]);
      graph2.push([
